...
import pandas as pd
from .common import iter_species_fastas, scan_pair, sequence_hash
...
def main():
    ...
    for species, acc, fp, header, seq in iter_species_fastas(Path(args.fasta_root)):
        ...
        for _, p in pairs.iterrows():
            ...
            base = {
                'species': species, 'accession': acc, 'sequence_hash': seq_hash,
                'pair_name': p['pair_name'], 'tag': p.get('tag', ''),
                ...
                'fwd_window': '' if f is None else f['window'],
                'rev_window': '' if r is None else r['window'],
                'fwd_ambiguity_followup': False if fl is None else bool(fl.fwd_ambiguity_followup),
                'rev_ambiguity_followup': False if fl is None else bool(fl.rev_ambiguity_followup),
            }
            out = dict(base)
