...
def summarize(calls_path: Path, outdir: Path):
    ...
    clean = (calls.groupby(['species','pair_name','accession'], as_index=False)
             .agg(N_raw_rows=('accession','size'), sequence_hash=('sequence_hash','first'),
                  informative_pair=('informative_pair','max'), eligible=('eligible','max'),
                  strict_hit=('strict_hit','max'), fail_MM=('fail_MM','max'),
                  fail_3p=('fail_3p','max'), product_len_bp=('product_len_bp','median'),
                  fwd_ambiguity_followup=('fwd_ambiguity_followup','max'),
                  rev_ambiguity_followup=('rev_ambiguity_followup','max'),
    ...
    summary = (clean.groupby(['species','pair_name'], as_index=False)
               .agg(N=('accession','nunique'), informative=('informative_pair','sum'),
                    ...
                    ambiguity_followup=('fwd_ambiguity_followup','sum')))
    ...
    summary['coverage_pct_all'] = np.where(summary['N']>0, 100*summary['covered']/summary['N'], np.nan)
    summary['delta_coverage_pp'] = summary['coverage_pct_eligible'] - summary['coverage_pct_all']
    summary.to_csv(outdir / 'species_pair_summary.csv', index=False)
    denominator = summary.rename(columns={'N':'N_total','informative':'N_informative',
                                          'eligible':'N_eligible','covered':'N_hits'})
    denominator[['species','pair_name','N_total','N_informative','N_eligible','N_hits']].to_csv(
        outdir / 'denominator_summary.csv', index=False)
    ...
    reason.to_csv(outdir / 'ineligibility_breakdown.csv', index=False)
    ...
    dup_summary = (dup.groupby(['species','pair_name'], as_index=False)
                   .agg(n_accessions=('accession','nunique'), total_rows=('rows_per_accession','sum'),
